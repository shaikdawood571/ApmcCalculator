...
function loadAdminBuyerTable() {
  const buyers = JSON.parse(localStorage.getItem("buyers") || "[]");
  const tbody = document.querySelector("#adminBuyerTable tbody");
  tbody.innerHTML = "";
  ...
  buyers.forEach((buyer, index) => {
    ...
    row.innerHTML = `
      ...
      <td>${buyer.paddy}</td>
      <td>₹${buyer.price}</td>
      <td>${buyer.approved ? "✅ Approved" : "❌ Pending"}</td>
      <td>
        ${!buyer.approved
          ? `<button onclick="approveBuyer(${index})">Approve</button>`
